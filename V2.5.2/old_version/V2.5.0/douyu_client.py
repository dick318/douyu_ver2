...
class MainWindow(MainWindowUi):
    ...
    def __init__(self):
        ...
        #self.tray_icon.action_about_software.triggered.connect(self.test_1)
    ...
    def display_list_message(self, data):    # 显示榜单消息
        try:
            ...
            if data['type'] == 'ranklist':    # 显示贡献榜
                self.list_widget.rank_day.list.setRowCount(len(data['list_day']))
                for i in range(len(data['list_day'])):    # 逐行添加显示信息
                    noble = (self.noble_dict[data['list_day'][i]['ne']]
                             if data['list_day'][i]['ne'] in self.noble_dict else '')
                    info_widget = UserInfoWidget(
                        str(i+1), noble,
                        data['list_day'][i]['level'], data['list_day'][i]['nickname'],
                        None, str(int(int(data['list_day'][i]['gold'])/100)))
                    self.list_widget.rank_day.list.setCellWidget(i, 0, info_widget)
                self.list_widget.rank_day.time.setText(u'更新时间：' + update_time)

                self.list_widget.rank_week.list.setRowCount(len(data['list']))
                for i in range(len(data['list'])):
                    noble = (self.noble_dict[data['list'][i]['ne']]
                             if data['list'][i]['ne'] in self.noble_dict else '')
                    info_widget = UserInfoWidget(
                        str(i+1), noble,
                        data['list'][i]['level'], data['list'][i]['nickname'],
                        None, str(int(int(data['list'][i]['gold'])/100)))
                    self.list_widget.rank_week.list.setCellWidget(i, 0, info_widget)
                self.list_widget.rank_week.time.setText(u'更新时间：' + update_time)

                self.list_widget.rank_all.list.setRowCount(len(data['list_all']))
                for i in range(len(data['list_all'])):
                    noble = (self.noble_dict[data['list_all'][i]['ne']]
                             if data['list_all'][i]['ne'] in self.noble_dict else '')
                    info_widget = UserInfoWidget(
                        str(i+1), noble,
                        data['list_all'][i]['level'], data['list_all'][i]['nickname'],
                        None, str(int(int(data['list_all'][i]['gold'])/100)))
                    self.list_widget.rank_all.list.setCellWidget(i, 0, info_widget)                 
                self.list_widget.rank_all.time.setText(u'更新时间：' + update_time)
                
            elif data['type'] == 'frank':    # 显示粉丝等级总榜
                self.list_widget.fans_all.list.setRowCount(len(data['list']))
                for i in range(len(data['list'])):
                    bl = ((' %s ' % data['list'][i]['bl'])
                          if len(data['list'][i]['bl']) == 1 else data['list'][i]['bl'])
                    brand = ' (%s) %s ' % (bl, data['bnn'])
                    info_widget = UserInfoWidget(
                        str(i+1), None,
                        data['list'][i]['lev'], data['list'][i]['nn'],
                        brand, str(int(int(data['list'][i]['fim'])/100)))
                    self.list_widget.fans_all.list.setCellWidget(i, 0, info_widget)
                    
                self.list_widget.fans_all.label.setText(u'粉丝总人数：' + data['fc'])
                self.list_widget.fans_all.time.setText(u'更新时间：' + update_time)
                
            elif data['type'] == 'fswrank':    # 显示粉丝7天亲密度排行
                self.list_widget.fans_week.list.setRowCount(len(data['list']))
                for i in range(len(data['list'])):
                    bl = ((' %s ' % data['list'][i]['bl'])
                          if len(data['list'][i]['bl']) == 1 else data['list'][i]['bl'])
                    brand = ' (%s) %s ' % (bl, data['bnn'])
                    info_widget = UserInfoWidget(
                        ...
                        data['list'][i]['lev'], data['list'][i]['nn'],
                        brand, str(int(int(data['list'][i]['fim'])/100)))
                    self.list_widget.fans_week.list.setCellWidget(i, 0, info_widget)
                    
                self.list_widget.fans_week.label.setText(u'粉丝总人数：' + data['fc'])
                self.list_widget.fans_week.time.setText(u'更新时间：' + update_time)
                
            elif data['type'] == 'online_noble_list':    # 显示在线贵族
                self.list_widget.noble_window.setTabText(0, u'贵族（%s）' % data['num'])

                self.list_widget.noble_list.list.setRowCount(len(data['nl']))
                for i in range(len(data['nl'])):
                    info_widget = UserInfoWidget(
                        None, self.noble_dict[data['nl'][i]['ne']],
                        data['nl'][i]['lv'], data['nl'][i]['nn'],
                        None, None)
                    self.list_widget.noble_list.list.setCellWidget(i, 0, info_widget)                    
                self.list_widget.noble_list.time.setText(u'更新时间：' + update_time)
