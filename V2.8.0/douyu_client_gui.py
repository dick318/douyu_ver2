...
MESSAGE_TYPE_LIST = (u'弹幕消息', u'进房消息', u'禁言消息', u'粉丝牌升级',
                     u'任免房管', u'房间礼物', u'广播礼物', u'批量礼物',
                     u'开通贵族', u'续费贵族', u'喇叭消息', u'系统广播',
                     u'开关播消息', u'用户信息')
...
class TopbarWidget(QFrame):    # 构建显示直播间信息的顶栏
    def __init__(self, parent=None):
        ...
        self.topbar_right_widget = QWidget(self)
        ...
        self.room_name.setCursor(IBEAM_CURSOR)
        self.cate_name.setCursor(IBEAM_CURSOR)
        self.room_status.setCursor(IBEAM_CURSOR)
        self.owner_name.setCursor(IBEAM_CURSOR)
        self.hot_value.setCursor(IBEAM_CURSOR)
        self.fans_num.setCursor(IBEAM_CURSOR)
        self.start_time.setCursor(IBEAM_CURSOR)

        self.room_name.setTextInteractionFlags(Qt.TextSelectableByMouse)    # 可选中标签上的文字
        self.cate_name.setTextInteractionFlags(Qt.TextSelectableByMouse)
        self.room_status.setTextInteractionFlags(Qt.TextSelectableByMouse)
        self.owner_name.setTextInteractionFlags(Qt.TextSelectableByMouse)
        self.hot_value.setTextInteractionFlags(Qt.TextSelectableByMouse)
        self.fans_num.setTextInteractionFlags(Qt.TextSelectableByMouse)
        self.start_time.setTextInteractionFlags(Qt.TextSelectableByMouse)

        connect_danmu_layout = QHBoxLayout()
        connect_danmu_layout.addWidget(QWidget())
        ...
        connect_danmu_layout.setSpacing(5)

        topbar_right_layout = QVBoxLayout()
        topbar_right_layout.addLayout(connect_danmu_layout)
        topbar_right_layout.addWidget(QWidget())
        topbar_right_layout.setContentsMargins(10, 10, 10, 10)

        topbar_details_layout = QGridLayout()
        topbar_details_layout.addWidget(self.room_name, 0, 0, 1, 3)
        topbar_details_layout.addWidget(self.cate_name, 1, 0, 1, 1)
        topbar_details_layout.addWidget(self.room_status, 1, 1, 1, 1)
        topbar_details_layout.addWidget(self.owner_name, 1, 2, 1, 1)
        topbar_details_layout.addWidget(self.hot_value, 2, 0, 1, 1)
        topbar_details_layout.addWidget(self.fans_num, 2, 1, 1, 1)
        topbar_details_layout.addWidget(self.start_time, 2, 2, 1, 1)
        topbar_layout_spacer = 5
        topbar_details_layout.setSpacing(topbar_layout_spacer)
        topbar_details_layout.setContentsMargins(10, 10, 10, 10)
        topbar_details_layout.setColumnStretch(2, 1)

        topbar_widget_layout = QHBoxLayout()
        topbar_widget_layout.addWidget(self.owner_avatar)
        topbar_widget_layout.addLayout(topbar_details_layout)
        topbar_widget_layout.addWidget(self.topbar_right_widget)
        topbar_widget_layout.setSpacing(0)
        topbar_widget_layout.setContentsMargins(0, 0, 0, 0)
      
        topbar_height = topbar_details_layout.sizeHint().height()
        self.owner_avatar.setFixedSize(topbar_height, topbar_height)
        self.owner_avatar.setIconSize(self.owner_avatar.size())
        self.owner_avatar.setToolTip(u'打开直播间')

        topbar_right_width = (self.roomid_label.width() +
                              self.roomid_enter.width() +
                              self.connect_danmu.width() +
                              10)
        self.topbar_right_widget.setLayout(topbar_right_layout)
        self.topbar_right_widget.setFixedSize(topbar_right_width, topbar_height)

        self.setLayout(topbar_widget_layout)
        self.resize(topbar_widget_layout.sizeHint())
...
class ConfigWidget(QWidget):    # 构建设置窗体
    def __init__(self, parent=None):
        ...
        self.record_all = QCheckBox(u'全选', self)
        ...
        self.record_bgbc = QCheckBox(u'批量礼物', self)
        ...
        record_message_type_layout.addWidget(self.record_blab, 1, 0)
        record_message_type_layout.addWidget(self.record_setadminres, 1, 1)
        record_message_type_layout.addWidget(self.record_dgb, 1, 2)
        record_message_type_layout.addWidget(self.record_spbc, 1, 3)
        record_message_type_layout.addWidget(self.record_bgbc, 2, 0)
        record_message_type_layout.addWidget(self.record_anbc, 2, 1)
        record_message_type_layout.addWidget(self.record_rnewbc, 2, 2)
        record_message_type_layout.addWidget(self.record_cthn, 2, 3)
        record_message_type_layout.addWidget(self.record_ssd, 3, 0)
        record_message_type_layout.addWidget(self.record_rss, 3, 1)        
        ...
        self.record_all.clicked.connect(self.check_all_event)
        self.record_chatmsg.clicked.connect(self.check_change_event)
        self.record_uenter.clicked.connect(self.check_change_event)
        self.record_newblackres.clicked.connect(self.check_change_event)
        self.record_blab.clicked.connect(self.check_change_event)
        self.record_setadminres.clicked.connect(self.check_change_event)
        self.record_dgb.clicked.connect(self.check_change_event)
        self.record_spbc.clicked.connect(self.check_change_event)
        self.record_bgbc.clicked.connect(self.check_change_event)
        self.record_anbc.clicked.connect(self.check_change_event)
        self.record_rnewbc.clicked.connect(self.check_change_event)
        self.record_cthn.clicked.connect(self.check_change_event)
        self.record_ssd.clicked.connect(self.check_change_event)
        self.record_rss.clicked.connect(self.check_change_event)

    def check_all_event(self, event=None):
        state = self.record_all.isChecked()
        self.record_chatmsg.setChecked(state)
        self.record_uenter.setChecked(state)
        self.record_newblackres.setChecked(state)
        self.record_blab.setChecked(state)
        self.record_setadminres.setChecked(state)
        self.record_dgb.setChecked(state)
        self.record_spbc.setChecked(state)
        self.record_bgbc.setChecked(state)
        self.record_anbc.setChecked(state)
        self.record_rnewbc.setChecked(state)
        self.record_cthn.setChecked(state)
        self.record_ssd.setChecked(state)
        self.record_rss.setChecked(state)

    def check_change_event(self, event=None):
        if (self.record_chatmsg.isChecked() and
            self.record_uenter.isChecked() and
            self.record_newblackres.isChecked() and
            self.record_blab.isChecked() and
            self.record_setadminres.isChecked() and
            self.record_dgb.isChecked() and
            self.record_spbc.isChecked() and
            self.record_bgbc.isChecked() and
            self.record_anbc.isChecked() and
            self.record_rnewbc.isChecked() and
            self.record_cthn.isChecked() and
            self.record_ssd.isChecked() and
            self.record_rss.isChecked()):
            self.record_all.setChecked(True)
        else:
            self.record_all.setChecked(False)
