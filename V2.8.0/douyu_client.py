...
class MainWindow(MainWindowUi):
    ...
    def __init__(self):
        ...
        # 定义各文本显示框中显示消息的类型
        self.danmu_message_type = ('chatmsg', 'uenter', 'newblackres',
                                   'blab', 'setadminres')    # 弹幕文本框显示的消息类型列表
        self.gift_message_type = ('dgb',)    # 礼物文本框显示的消息类型列表
        self.broadcast_message_type = ('spbc', 'bgbc', 'anbc',
                                       'rnewbc', 'cthn', 'ssd')    # 广播文本框显示的消息类型列表
        self.list_message_type = ('ranklist', 'frank', 'fswrank',
                                  'noble_num_info', 'online_noble_list')    # 榜单窗体显示的消息类型
...
#################################################################################################
    ...
    def thread_display_message(self, queue_data, queue_order_except):    # 触发显示线程
        ...
        self.process_message_signal.emit({'type':'disconnected', 'from':'main'})
    ...
    def keeplive_timeout_event(self):    # 定时器的事件处理器：发生服务器心跳异常
        ...
        self.is_error = True
        self.can_refresh_details = False
        self.update_title_statusbar_tray(self.room_icon, u'服务器心跳异常')    # 设置窗体标题、状态栏、托盘图标说明
    ...
    def thread_live_timeout_event(self):    # 定时器的事件处理器：发生程序内心跳异常
        ...
        self.is_error = True
        self.can_refresh_details = False
        self.update_title_statusbar_tray(self.room_icon, u'程序内部心跳异常')    # 设置窗体标题、状态栏、托盘图标说明
            ...
            if os.path.exists(db_path):    # 数据库是否存在
                try:
                    ...
                    msg_type_dict = {u'弹幕消息':'chatmsg', u'进房消息':'uenter',
                                     ...
                                     u'广播礼物':'spbc', u'批量礼物':'bgbc',
                                     u'开通贵族':'anbc', u'续费贵族':'rnewbc',
                                     u'喇叭消息':'cthn', u'系统广播':'ssd',
                                     u'开关播消息':'rss', u'用户信息':'user'}
                    condition = {}    # 要查询的条件
    ...
    def query_user_info(self, database, condition):    # 查询用户信息   
        args_list = [('table_chatmsg', 'uid', 'nn'), ('table_uenter', 'uid', 'nn'),
                     ...
                     ('table_rnewbc', 'uid', 'unk'), ('table_spbc', 'sid', 'sn'),
                     ('table_bgbc', 'sid', 'sn')]      
        result_list = []
