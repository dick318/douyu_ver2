...
class MainWindow(MainWindowUi):
    ...
    def __init__(self):
        ...
        self.danmu_message_type = (
            'chatmsg', 'uenter', 'newblackres', 'blab', 'setadminres')    # 弹幕文本框显示的消息类型列表
        self.gift_message_type = ('dgb',)    # 礼物文本框显示的消息类型列表
        self.broadcast_message_type = (
            'spbc', 'bgbc', 'anbc', 'rnewbc', 'cthn', 'ssd')    # 广播文本框显示的消息类型列表
        self.list_message_type = (
            'ranklist', 'frank', 'fswrank', 'noble_num_info', 'online_noble_list')    # 榜单窗体显示的消息类型
        self.all_message_type = (
            self.danmu_message_type + self.gift_message_type + self.broadcast_message_type +
            self.list_message_type + ('rss', 'error', 'loginres', 'keeplive'))
        ...
        self.table_dgb = ('time INT', 'type TEXT', 'rid TEXT', 'gfid TEXT', 
                          ...
                          'gfcnt TEXT', 'hits TEXT', 'bcnt TEXT', 'rg TEXT',
                          'nl TEXT', 'bnn TEXT', 'bl TEXT', 'brid TEXT',
                          'noble TEXT', 'gn TEXT')
        self.table_spbc = ('time INT', 'type TEXT', 'sn TEXT', 'dn TEXT',
                           'gn TEXT', 'gc TEXT', 'drid TEXT', 'gb TEXT',
                           'es TEXT', 'gfid TEXT', 'sid TEXT')
        self.table_bgbc = ('time INT', 'type TEXT', 'gfid TEXT', 'gc TEXT',
                           'drid TEXT', 'sid TEXT', 'sn TEXT', 'did TEXT',
                           'dn TEXT', 'gn TEXT')        
        ...
        self.message_table_key = {
            ...
            'dgb': ('time', 'type', 'rid', 'gfid', 'uid', 'bg', 'nn', 'level', 'gfcnt', 
                    'hits', 'bcnt', 'rg', 'nl', 'bnn', 'bl', 'brid', 'noble', 'gn'),
            'spbc': ('time', 'type', 'sn', 'dn', 'gn', 'gc', 'drid', 'gb', 'es', 'gfid', 'sid'),            
            'bgbc': ('time', 'type', 'gfid', 'gc', 'drid', 'sid', 'sn', 'did', 'dn', 'gn'),
        ...
        self.set_record_bgbc = False    # 设置是否记录批量礼物
        ...
        # 绑定信号与槽
        self.keeplive_timer = QTimer(self)    # 创建用于判断是否发生服务器心跳异常的定时器
        self.keeplive_timer.timeout.connect(self.keeplive_timeout_event)
        self.thread_live_timer = QTimer(self)    # 创建用于判断是否发生程序内心跳异常的定时器
        self.thread_live_timer.timeout.connect(self.thread_live_timeout_event)
...
#################################################################################################
    ...
    def changed_config_signal_connect_slot(self):    # 设置组件的信号绑定槽
        ...
        self.config_widget.record_bgbc.clicked.connect(self.changed_config_event)
    ...
    def changed_config_signal_disconnect_slot(self):    # 设置组件的信号取消绑定槽
        ...
        self.config_widget.record_bgbc.clicked.disconnect(self.changed_config_event)
        ...
        try:
            ...
            if msg['type'] == 'chatmsg':    # 弹幕消息
                ...
                if msg['gfcnt'] == '1':
                    ...
                    text.append(u'<span class="gfcnt">×%s</span>' % msg['gfcnt'])                    
            elif msg['type'] == 'spbc'or msg['type'] == 'bgbc':    # 广播礼物消息
                url = (self.douyu_url + msg['drid']) if msg['drid'] != '0' else ''
                text.append(u'<span class="sn">%s</span>' % msg['sn'])
                text.append(u'<span class="txt">赠送给</span>')
                text.append(u'<span class="dn">%s</span>' % msg['dn'])
                text.append(u'<span class="txt">%s个</span>' % msg['gc'])
                text.append(u'<span class="gn">%s</span>' % msg['gn'])
    ...
    def thread_get_details(self, queue_recv):    # 获取直播间详细信息的线程
        ...
        while self.run_get_details:
            ...
            if data_recv['type'] == 'close':    # 结束线程
                break
            elif data_recv['type'] == 'update':    # 获取对应直播间信息
                room_id = data_recv['rid']
                    ...
                    if html_dict and type(html_dict) == type(dict()):    # 获取html数据成功，提取信息
                        ...
                        if error == 0 and data != 'ERROR':    # 成功获取到直播间详细信息
                            ...
                            data_send = {
                                'time': int(time.time()),
                                'type': 'error',
                                'data': {'code':error, 'status':status_text},
                                'rid': room_id
                            }
    ...
    def update_details_event(self, data_recv):    # 更新直播间详细信息的处理器：由获取信息线程触发
        if data_recv:
            time_short = time.strftime(' (%H:%M:%S) ', time.localtime(data_recv['time']))
            if data_recv['type'] == 'error':    # 获取信息时发生错误
                status_str = data_recv['data']['status'] + time_short
                self.statusbar.get_html_status.setToolTip(status_str)
                self.statusbar.get_html_status.setText(status_str)    # 状态栏显示错误信息
                ...
                time_str = time.strftime('[%Y-%m-%d %H:%M:%S]', time.localtime(data_recv['time']))
                ...
                if data_recv['data']['room_name'] != self.last_title:    # 上一个标题不为空，不是第一次更新直播间信息                
                    if self.last_title:
                        text_html = (self.message_css + '<p>' + time_str +
    ...
    def create_douyu_table(self, db):    # 创建对应直播间的各类型消息的数据库表格
        tables = (('table_chatmsg', self.table_chatmsg),
                  ('table_uenter', self.table_uenter),
                  ('table_newblackres', self.table_newblackres),
                  ('table_blab', self.table_blab),
                  ('table_setadminres', self.table_setadminres),
                  ('table_dgb', self.table_dgb),
                  ('table_spbc', self.table_spbc),
                  ('table_bgbc', self.table_bgbc),
                  ('table_anbc', self.table_anbc),
                  ('table_rnewbc', self.table_rnewbc),
                  ('table_cthn', self.table_cthn),
                  ('table_ssd', self.table_ssd),
                  ('table_rss', self.table_rss))
        result = []
        for name, columns in tables:    # 逐一创建表格并检查，返回错误的表格名
            if not db.create(name, columns):
                result.append(name)
        return result
    ...
    def thread_record_message(self, queue, room_id):    # 记录消息的线程
        ...
        result = self.create_douyu_table(database_record)    # 数据库中不存在表格则创建
        if result:    # 存在错误的表格
            self.record_warn_signal.emit({
                'time': int(time.time()),
                'type': 'error',
                'data': u'数据库的表格错误：%s' % repr(result),
                'rid': room_id
            })
            ...
            if os.path.exists(db_path):    # 数据库是否存在
                try:
                    ...
                    condition_dic = {
                        ...
                        'spbc': {'sid': uid, 'sn': nn, 'gn': gn, 'dn': dn},
                        'bgbc': {'sid': uid, 'sn': nn, 'gn': gn, 'dn': dn},
    ...
    def thread_query_message(self, db_path, msg_type, condition, max_result):    # 查询线程
        try:
            ...
            if msg_type in self.message_table_key:    # 查询消息
                ...
                if msg_type in ('anbc', 'rnewbc') and 'unk' in condition:
                    condition2 = {'gvnk': condition['unk']}
                    condition2.update(condition)
                    del condition2['unk']
                    result_all += database.query_all(table_name, '*', condition2)    # 返回数据元组的列表
    ...
    def query_user_info(self, database, condition):    # 查询用户信息   
        args_list = [('table_chatmsg', 'uid', 'nn'),
                     ('table_uenter', 'uid', 'nn'),
                     ('table_dgb', 'uid', 'nn'),
                     ('table_newblackres', 'sid', 'snic'),
                     ('table_newblackres', 'did', 'dnic'),
                     ('table_blab', 'uid', 'nn'),
                     ('table_setadminres', 'userid', 'adnick'),
                     ('table_anbc', 'uid', 'unk'),
    ...
    def get_config_data_from_variables(self):    # 从用户设置变量的值获取设置数据
        return {
            ...
            'RecordBgbc': self.set_record_bgbc,
    ...
    def get_config_data_from_windows(self):    # 从设置窗体的各组件获取用户更改后的设置数据
        return {
            ...
            'RecordBgbc': self.config_widget.record_bgbc.isChecked(),
    ...
    def set_config_data_to_variables(self, config_data):    # 根据所提供的配置数据设置用户设置变量的值
        try:
            ...
            self.set_record_bgbc = config_data['RecordBgbc']
            ...
            self.config_widget.check_change_event()
            return True
        except Exception as exc:
            exc_msg = exception_message(exc)
            ERROR_LOGGER.error(exc_msg)
    ...
    def set_config_data_to_windows(self, config_data):    # 根据所提供的配置数据配置设置窗体中各组件的值
        try:
            ...
            self.config_widget.record_bgbc.setChecked(config_data['RecordBgbc'])
    ...
    def set_record_type(self):    # 设置记录消息的类型列表
        config_data = {
            'chatmsg': self.set_record_chatmsg,
            'uenter': self.set_record_uenter,
            'newblackres': self.set_record_newblackres,
            'blab': self.set_record_blab,
            'setadminres': self.set_record_setadminres,
            'dgb': self.set_record_dgb,
            'spbc': self.set_record_spbc,
            'bgbc': self.set_record_bgbc,
            'anbc': self.set_record_anbc,
            'rnewbc': self.set_record_rnewbc,
            'cthn': self.set_record_cthn,
            'ssd': self.set_record_ssd,
            'rss': self.set_record_rss
            }
        self.record_type_list = []
        for key in config_data:    # 根据勾选框的值设置记录消息类型列表
            if config_data[key]:
                self.record_type_list.append(key)
