...
class MainWindow(MainWindowUi):
    ...
    def __init__(self):
        ...
        # 各文件的路径
        self.emotion_dir = os.path.join(CONFIG_FOLDER, EMOTION_FOLDER)
        self.noble_dir = os.path.join(CONFIG_FOLDER, NOBLE_FOLDER)
        ...
        self.list_message_type = ('ranklist', 'frank', 'noble_num_info', 'online_noble_list')    # 榜单窗体显示的消息类型
        ...
        self.client = {'0': u'浏', '1': u'安', '2': u'苹', '14': u'电'}
        self.noble = {'1': u'骑', '2': u'子', '3': u'伯', '4': u'公',
                      '5': u'王', '6': u'皇', '7': u'侠'}
        ...
        self.set_record_all = True    # 设置是否记录全部消息
        self.set_record_chatmsg = True    # 设置是否记录弹幕消息
        self.set_record_uenter = True    # 设置是否记录进房消息
        self.set_record_newblackres = True    # 设置是否记录禁言消息
        self.set_record_blab = True    # 设置是否记录粉丝牌升级消息
        self.set_record_setadminres = True    # 设置是否记录任免房管消息
        self.set_record_dgb = True    # 设置是否记录房间礼物
        self.set_record_spbc = True    # 设置是否记录广播礼物
        self.set_record_anbc = True    # 设置是否记录开通贵族
        self.set_record_rnewbc = True    # 设置是否记录续费贵族
        self.set_record_cthn = True    # 设置是否记录喇叭消息
        self.set_record_ssd = True    # 设置是否记录系统广播
        self.set_record_rss = True    # 设置是否记录开关播消息
...
#################################################################################################
    ...
    def display_list_message(self, data):    # 显示榜单消息
        ...
        display_text = self.get_list_text(data)
        if display_text != 'ERROR':
            if data['type'] == 'ranklist':    # 显示贡献榜
                self.list_widget.list_day.list.setText(display_text[0])
                self.list_widget.list_week.list.setText(display_text[1])
                self.list_widget.list_all.list.setText(display_text[2])
                self.list_widget.list_day.time.setText(u'更新时间：' + update_time)
                self.list_widget.list_week.time.setText(u'更新时间：' + update_time)
                self.list_widget.list_all.time.setText(u'更新时间：' + update_time)
            elif data['type'] == 'frank':    # 显示粉丝团
                self.list_widget.list_fans.list.setText(display_text)
                self.list_widget.list_fans.label.setText(u'粉丝总人数：' + data['fc'])
                self.list_widget.list_fans.time.setText(u'更新时间：' + update_time)
            elif data['type'] == 'online_noble_list':    # 显示在线贵族
                self.list_widget.noble_window.setTabText(0, u'贵族（%s）' % data['num'])
                self.list_widget.list_noble.list.setText(display_text)
                self.list_widget.list_noble.time.setText(u'更新时间：' + update_time)
            elif data['type'] == 'noble_num_info':    # 显示在线贵族各等级数量
                self.list_widget.list_noble.label.setText(display_text)
        ...
        try:
            ...
            if msg['type'] == 'chatmsg':    # 弹幕消息
                ...
                text.append(u'<span class="ct">[%s]</span>' %
                            (self.client[msg['ct']] if msg['ct'] in self.client else '未'))
                text.append((u'<span class="rg">&nbsp;房&nbsp;</span>') if msg['rg'] == '4' else '')
                text.append((u'<span class="nl">&nbsp;%s&nbsp;</span>' %
                             self.noble[msg['nl']]) if msg['nl'] in self.noble else '')
                ...
                for emot in emot_list:
                    img_num = int(emot[8:-1])
                    if (img_num >= 1 and img_num <= 17) or (img_num >= 101 and img_num <= 137):
                        img = ('<img class="emotion" src="%s" />' %
                               (os.path.join(self.emotion_dir, emot[6:-1] + '.png')))
    ...
    def get_list_text(self, msg):    # 获取可用于在榜单文本框中显示的html文本
        try:
            if msg['type'] == 'ranklist':    # 贡献榜消息
                lists = []
                for li in (msg['list_day'], msg['list'], msg['list_all']):
                    user_list = self.message_css
                    for user in li:
                        text = []
                        space = '&nbsp;' * (4 - len(user['level']))
                        text.append((u'<span class="nl">&nbsp;%s&nbsp;</span>' %
                                     self.noble[user['ne']]) if user['ne'] in self.noble else '')
                        text.append(u'<span class="level">%s</span>' % (space + user['level'] + space))
                        text.append(u'<span class="black">%s</span>' % user['nickname'])
                        text.append(u'<span class="grey">(%s)</span>' % int(int(user['gold'])/100))
                        user_list += '<p>' + '\n'.join(text) + '</p>'
                    lists.append(user_list)
                return lists                    
            elif msg['type'] == 'frank':    # 粉丝团消息
                fans_list = self.message_css
                for user in msg['list']:
                    text = []
                    space = '&nbsp;' * (4 - len(user['lev']))
                    bl = ('&nbsp;%s&nbsp;' % user['bl']) if len(user['bl']) == 1 else user['bl']
                    text.append(u'<span class="bnn">&nbsp;(%s)%s&nbsp;</span>' % (bl, msg['bnn']))
                    text.append(u'<span class="level">%s</span>' % (space + user['lev'] + space))
                    text.append(u'<span class="black">%s</span>' % user['nn'])
                    text.append(u'<span class="grey">(%s)</span>' % int(int(user['fim'])/100))
                    fans_list += '<p>' + '\n'.join(text) + '</p>'
                return fans_list
            elif msg['type'] == 'online_noble_list':    # 在线贵族消息
                noble_list = self.message_css
                for user in msg['nl']:
                    text = []
                    space = '&nbsp;' * (4 - len(user['lv']))
                    text.append(u'<span class="nl">&nbsp;%s&nbsp;</span>' % self.noble[user['ne']])
                    text.append(u'<span class="level">%s</span>' % (space + user['lv'] + space))
                    text.append(u'<span class="black">%s</span>' % user['nn'])
                    noble_list += '<p>' + '\n'.join(text) + '</p>'
                return noble_list
            elif msg['type'] == 'noble_num_info':    # 在线贵族数量消息
                noble_msg = []
                for user in msg['list']:
                    noble_msg.append(u'[%s]%s' % (self.noble[user['lev']], user['num']))
                return '  '.join(noble_msg)
        except Exception as exc:
            exc_msg = exception_message(exc)
            ERROR_LOGGER.error(exc_msg)
            ERROR_LOGGER.error(repr(msg))
    ...
    def query_button_event(self):    # 查询按钮的事件处理器        
        ...
        if rid:
            ...
            # 不同类型的消息对应不同的查询条件组合
            condition_dic = {
                'chatmsg':{'client':client, 'noble':noble, 'nn':nn, 'txt':txt}, 
                'uenter':{'noble':noble, 'nn':nn}, 
                'newblackres':{'dnic':nn},
                'blab':{'nn':nn},
                'setadminres':{'adnick':nn},
                'dgb':{'noble':noble, 'nn':nn, 'gn':gn}, 
                'spbc':{'sn':nn, 'gn':gn, 'dn':dn}, 
                'anbc':{'unk':nn, 'donk':dn, 'noble':noble},
                'rnewbc':{'unk':nn, 'donk':dn, 'noble':noble},
                'cthn':{'unk':nn, 'onk':dn},
                'ssd':{},
                'rss':{}}
    ...
    def get_default_config_data(self):    # 默认的设置数据
        return {
            ...
            'RecordChatmsg': True,
            'RecordUenter': True,
            'RecordNewblackres': True,
            'RecordBlab': True,
            'RecordSetadminres': True,
            'RecordDgb': True,
            'RecordSpbc': True,
            'RecordAnbc': True,
            'RecordRnewbc': True,
    ...
    def set_advanced_settings(self, config_data):    # 根据所提供的配置数据设置高级设置变量的值
        try:
