...
class ProcessDanmuServerData(object):    # 处理从弹幕服务器接收到的各种信息数据
    def __init__(self, queue_data, queue_message=None,
        ...
        self.show_function = {
            ...
            'blab': self.show_blab, 'rnewbc': self.show_rnewbc,
            'rss': self.show_rss, 'error': self.show_error,
            'cthn': self.show_cthn, 'ssd': self.show_ssd,
            'ranklist': self.show_ranklist, 'frank': self.show_frank,
    ...
    def parse_msg_utf8(self, msg_dict):    # 提取消息类型，并调用不同的处理方法
        ...
        if '/type@=' in msg_utf8_str:
            try:
                ...
                #log_msg_parse(msg_utf8_str, ('uenter', 'chatmsg'))                
            except Exception as exc:
                exc_msg = exception_message(exc)
                ERROR_LOGGER.error(exc_msg)
        ...
        try:
            ...
            if hits == '0':
                ...
                if noble_name:
                    ...
                    msg_show = u'%s [%s] %s 赠送给主播 %s X%s' % (time_recv, level, nn, giftname, hits)
            else:
                msg_show = None
    ...
    def show_spbc(self, msg):    # 礼物广播消息
        try:
            time_int10 = msg['time']
            time_recv = time.strftime('[%Y-%m-%d %H:%M:%S]', time.localtime(msg['time']))
            sn = re.search('/sn@=(.*?)/', msg['data']).group(1) if '/sn@=' in msg['data'] else ''    # 赠送用户名称
            ...
            gb = re.search('/gb@=(.*?)/', msg['data']).group(1) if '/gb@=' in msg['data'] else '0'    # 是否有宝箱，0-无 1-有
        ...
        try:
            ...
            for user in flist:
                if user:
                    ...
                    fans.append({'uid': uid, 'nn': nn, 'fim': fim, 'bl': bl, 'lev': lev})
